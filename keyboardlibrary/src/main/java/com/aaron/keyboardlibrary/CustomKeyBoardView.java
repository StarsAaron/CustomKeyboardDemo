...
public class CustomKeyBoardView extends KeyboardView {
    ...
    public void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        mKeyBoard = KeyboardManager.getKeyBoardType();
        List<Key> keys = mKeyBoard.getKeys();
        for (Key key : keys) {
            // 数字键盘的处理
            if (mKeyBoard.equals(KeyboardManager.numKeyboard)) {
                initRightType(key);
                drawNumSpecialKey(key, canvas);
            } else if (mKeyBoard.equals(KeyboardManager.abcKeyboard)) {
                drawABCSpecialKey(key, canvas);
            } else if (mKeyBoard.equals(KeyboardManager.symbolKeyboard)) {
